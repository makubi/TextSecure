...
public class MessageNotifier {
  ...
  public static final int ANDROID_BUG_130689_NOTIFICATION_ID = NOTIFICATION_ID + 100;
  ...
  private static Optional<NotificationState> currentNotificationState(@NonNull  Context context,
    ...
    try {
      ...
      if ((telcoCursor == null || telcoCursor.isAfterLast()) &&
        ...
        NotificationManagerCompat.from(context).cancel(ANDROID_BUG_130689_NOTIFICATION_ID);
                                                   ...
                                                   boolean displayWearableNotifications)
  {
    ...
    if(hasWearSupport()) {
      // Always cancel summary notification, so it gets also closed if only one unread thread exists after more than one was unread
      NotificationManagerCompat.from(context).cancel(NOTIFICATION_ID);
      NotificationManagerCompat.from(context).cancel(ANDROID_BUG_130689_NOTIFICATION_ID);
  ...
  /*
    TODO check in master: On 4.4, the text without sender is shown
   */
                                                     ...
                                                     boolean displayWearableNotifications)
  {
    ...
    if (signal) {
      ...
      if(displayBugfixNotification) {
        NotificationCompat.Builder bugFixBuilder = new Bug130689NotificationFixBuilder(context);

        NotificationManagerCompat.from(context).notify(ANDROID_BUG_130689_NOTIFICATION_ID, bugFixBuilder.build());
      }
    }
    ...
    if(displayWearableNotifications) {
      ...
      // Display the summary as normal notification
      builder.setGroup("");
